...
describe Isomorfeus::Hamster do
  let(:env) { Isomorfeus::Hamster.new(path) }
  after     { env.close rescue nil }
  let(:db)  { env.database }

  it 'has version constants' do
    Isomorfeus::Hamster::LIB_VERSION_MAJOR.should be_instance_of(Integer)
    Isomorfeus::Hamster::LIB_VERSION_MINOR.should be_instance_of(Integer)
    Isomorfeus::Hamster::LIB_VERSION_PATCH.should be_instance_of(Integer)
    Isomorfeus::Hamster::LIB_VERSION.should be_instance_of(String)
    Isomorfeus::Hamster::VERSION.should be_instance_of(String)
  end

  describe Isomorfeus::Hamster::Environment do
    subject { env }
    ...
    describe 'new' do
      it 'returns environment' do
        env = Isomorfeus::Hamster::Environment.new(path)
        env.should be_instance_of(described_class)
        env.close
      end

      it 'accepts block' do
        Isomorfeus::Hamster::Environment.new(path) do |env|
          env.should be_instance_of(described_class)
          42
        end.should == 42
      end
      ...
      it 'accepts options' do
        env = Isomorfeus::Hamster::Environment.new(path, :nosync => true, :mode => 0777, :maxreaders => 777, :mapsize => 111111, :maxdbs => 666)
        env.should be_instance_of(described_class)
        ...
        env = Isomorfeus::Hamster::Environment.new(path, :nosync => false)
    ...
    describe Isomorfeus::Hamster::Transaction do
      subject { env }
  ...
  describe Isomorfeus::Hamster::Database do
    subject { db }
    ...
    it 'should support named databases' do
      main = env.database
      db1 = env.database('db1', :create => true, :dupsort => true) # dupsort just to test multiple args
      db2 = env.database('db2', :create => true)
      db3 = env.database('db3', create: true)
      db4 = env.database('db4', create: true, dupsort: true) # test kwarg style
    ...
    it 'should delete by key' do
      proc { subject.delete('cat') }.should raise_error(Isomorfeus::Hamster::Error::NOTFOUND)
      proc { subject.delete('cat', 'garfield') }.should raise_error(Isomorfeus::Hamster::Error::NOTFOUND)
  ...
  describe Isomorfeus::Hamster::Cursor do
    before do
      db.put('key1', 'value1')
      db.put('key2', 'value2')
    end
    ...
    it 'should complain setting a key-value pair without dupsort' do
      db.cursor do |c|
        proc { c.set('key1', 'value1') }.should raise_error(Isomorfeus::Hamster::Error)
      end
    end

    it 'should raise without block or txn' do
      proc { db.cursor.next }.should raise_error(Isomorfeus::Hamster::Error)
    end

    it 'should raise outside txn' do
      c = nil
      env.transaction { c = db.cursor }
      proc { c.next }.should raise_error(Isomorfeus::Hamster::Error)
    end
